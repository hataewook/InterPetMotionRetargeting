...
{
    /// <summary>
    /// Rig-free variant of <see cref="SmalRetargeter"/>: driven straight from a *baked*
    /// clip JSON (the one <c>Python/export_smal_unity_clip_rigfree.py</c> produces) with
    /// NO source rig and NO runtime calibration. The exporter bakes the SMAL->Unity
    /// coordinate conversion (rotation + handedness + scale) into the clip, so the joint
    /// positions this reconstructs by forward kinematics are already in Unity space — the
    /// legs come out the right way round without fitting to an imported skeleton.
    ///
    /// Contrast with <see cref="SmalRetargeterDirect"/>, which reads a RAW
    /// (<c>export_smal_unity_clip.py</c>) clip and needs a calibration rig to recover the
    /// handedness the raw clip omits. Here that recovery lives in the Python export instead,
    /// so nothing extra is needed in the scene.
    ///
    /// Each frame: reconstruct every joint from the clip's rest skeleton
    /// (<c>restJoints</c> + <c>parents</c>) and its per-frame world-space rotation deltas
    ///   pos[child] = pos[parent] + delta[parent] * (rest[child] - rest[parent])
    /// (the delta is a world rotation, so no accumulation), optionally compose the captured
    /// global body transform, then retarget. The solve pipeline (torso frame, two-bone leg
    /// IK, CCD trunk, per-segment spine/neck/tail FK, toe FK, copy-rotation, foot lock,
    /// grounding) is identical to <see cref="SmalRetargeter"/>.
    ///
    /// Caveats:
    ///   * Copy-rotation (jaw / ears) uses the joint's world delta-from-rest (Unity space),
    ///     not an FBX bone's local frame, so the per-axis remap (mapX/Y/Z) needs re-tuning
    ///     from the rig-path values.
    ///   * The clip must be marked <c>space="unity"</c> (i.e. from the rig-free exporter);
    ///     a raw clip plays mirrored.
    ///
    /// Runs in LateUpdate: advances its own playback clock, reconstructs the pose, then
    /// retargets. Attach to the target root and assign the baked clip JSON.
    /// </summary>
    public class SmalRetargeterRigFree : MonoBehaviour, IBodyFrameProvider, IGroundable
    {
        [Header("Source clip")]
        [Tooltip("Baked JSON produced by Python/export_smal_unity_clip_rigfree.py (already " +
                 "in Unity space). A raw export_smal_unity_clip.py clip will retarget mirrored.")]
        [SerializeField] TextAsset clipJson;
        [Tooltip("Optional transform whose placement/orientation/scale positions the " +
                 "reconstructed SMAL skeleton in the scene. Leave empty to keep it at the " +
                 "clip's own (Unity-space) origin — fine for articulation-only retargets.")]
        ...
        [Tooltip("Bake the clip's captured global body transform (turn / translate / scale) " +
                 "into the reconstructed joint positions. Disable to keep the SMAL body " +
                 "planted and reconstruct only its articulation.")]
        ...
        [Tooltip("Copy a SMAL joint's world delta-from-rest (jaw 32, ears 33/34) onto a " +
                 ...
                 "delta is world-space (Unity), not the target bone's local frame, so the " +
                 "remap generally differs from the rig-path values. Leave target empty to skip.")]
        ...
        class Clip
        {
            ...
            public string space;       // "unity" for a baked (rig-free) clip
            public int[] parents;
        ...
        Clip clip;
        Vector3[] restJoint;           // [joint] rest position (Unity space, from the clip)
        int[] jointParent;             // [joint] parent index (-1 = root)
        Quaternion[] armDelta;         // [joint] current-frame world delta-from-rest
        ...
        void Awake()
        {
            ...
            if (clipJson == null)
                throw new MissingReferenceException("SmalRetargeterRigFree: no clip JSON assigned");

            clip = JsonUtility.FromJson<Clip>(clipJson.text);
            if (clip == null || clip.frameCount <= 0 || clip.jointCount <= 0)
                throw new InvalidOperationException("SmalRetargeterRigFree: clip JSON failed to parse");
            if (clip.parents == null || clip.parents.Length < clip.jointCount
                ...
                throw new InvalidOperationException(
                    "SmalRetargeterRigFree: clip is missing parents / restJoints (re-export the clip)");
            if (clip.space != "unity")
                Debug.LogWarning("SmalRetargeterRigFree: clip is not marked space=\"unity\" — it was " +
                    "probably made with export_smal_unity_clip.py (raw SMAL space) and will retarget " +
                    "mirrored. Use export_smal_unity_clip_rigfree.py, or SmalRetargeterDirect with a " +
                    "calibration rig.");
            ...
            if (logCalibration)
                Debug.Log($"SmalRetargeterRigFree: loaded clip '{clip.name}' " +
        ...
        void Start()
        {
            ...
            toeBindRel = new Quaternion[legs.Length];
            {
                ...
                for (int i = 0; i < legs.Length; i++)
                {
                    ...
                    if (segDir.sqrMagnitude < Eps)
                        throw new InvalidOperationException(
                            $"SmalRetargeterRigFree: leg '{legs[i].name}' toe segment is degenerate at bind");
            ...
            for (int i = 0; i < fkChains.Length; i++)
            {
                ...
                for (int k = 0; k < bones.Length; k++)
                {
                    ...
                    if (segDir.sqrMagnitude < Eps)
                        throw new InvalidOperationException(
                            $"SmalRetargeterRigFree: FK chain '{fkChains[i].name}' segment {seg} is degenerate at bind");
                    fkSegIndex[i][k] = seg;
            ...
            if (logCalibration)
                Debug.Log($"SmalRetargeterRigFree: rest leg lengths=[{string.Join(", ", legLenTarget)}]");
        }
        ...
        /// <summary>
        /// Reconstruct every joint's world position from the (Unity-space, baked) clip. The
        /// clip stores a world-space rotation delta-from-rest per joint, so a joint does not
        /// accumulate its ancestors' rotations: the segment parent->child is rotated by the
        /// PARENT's delta alone. Positions therefore come from
        ///   pos[child] = pos[parent] + delta[parent] * (rest[child] - rest[parent])
        /// walked in joint order (parents precede children in SMAL order). The captured
        /// global body transform is optionally composed on top, then the whole skeleton is
        /// placed into the scene through <see cref="smalSpaceRoot"/> (identity if none).
        /// </summary>
        void ComputePose(int f, int next, float frac)
        {
            ...
            // Captured global body transform: rotate/scale/translate the whole skeleton.
            // Copy-rotation stays on the pre-global per-joint delta, so it is unaffected —
            // matching the rig path where a joint's LOCAL rotation is independent of the
            ...
            if (applySourceGlobalMotion)
            {
                ...
                for (int j = 0; j < n; j++)
                    smalWorldPos[j] = gRot * (gScale * smalWorldPos[j]) + gPos;
            }
        ...
        void ResolveTargetRig()
        {
            Transform Require(Transform bone, string mapping)
            {
                if (bone == null)
                    throw new MissingReferenceException(
                        $"SmalRetargeterRigFree: target mapping '{mapping}' has no bone assigned");
                if (bone != targetRoot && !bone.IsChildOf(targetRoot))
                    throw new InvalidOperationException(
                        $"SmalRetargeterRigFree: mapped bone '{bone.name}' is not under " +
            ...
            for (int i = 0; i < fkChains.Length; i++)
            {
                ...
                if (fk.joints == null || fk.joints.Length < 2)
                    throw new MissingReferenceException(
                        $"SmalRetargeterRigFree: FK chain '{fk.name}' needs at least two SMAL joints " +
        ...
        static void RequireDescendant(Transform ancestor, Transform descendant, string name)
        {
            ...
            throw new MissingReferenceException(
                $"SmalRetargeterRigFree: leg '{name}' bones are not a parent->child chain " +
        ...
        static Transform[] BuildChain(Transform baseBone, Transform effector, string name)
        {
            ...
            if (chain.Count == 0 || chain[chain.Count - 1] != baseBone)
                throw new MissingReferenceException(
                    $"SmalRetargeterRigFree: chain '{name}' is not a contiguous chain from " +
        ...
        void LateUpdate()
        {
            ...
            for (int i = 0; i < legs.Length; i++)
            {
                ...
                if (srcLen < Eps)
                    throw new InvalidOperationException($"SmalRetargeterRigFree: leg '{leg.name}' collapsed");
            ...
            for (int i = 0; i < chains.Length; i++)
            {
                ...
                if (srcLen < Eps)
                    throw new InvalidOperationException($"SmalRetargeterRigFree: chain '{chain.name}' collapsed");
        ...
        /// clip's per-joint delta is already relative to the rest pose (neutral = identity),
        /// so it is used directly as the axis-angle articulation: remap the axis into the
        /// target bone's frame, scale by the weight, add to the target bind. NOTE: the delta
        /// is world-space (Unity), so the remap differs from the FBX rig-path values.</summary>
                               ...
                               out Quaternion rot, out Vector3 origin, out float len)
        {
            ...
            if (fwd.sqrMagnitude < Eps || up.sqrMagnitude < Eps)
                throw new InvalidOperationException("SmalRetargeterRigFree: degenerate torso frame");

            rot = Quaternion.LookRotation(fwd, up);
        }

        /// <summary>Reconstructed SMAL joint position, One-Euro-smoothed and cached once per
        /// LateUpdate so every reader in a frame sees the same filtered value and the filter
        /// advances exactly once per joint per frame.</summary>
